// jianzhioffer数组中只出现一次的数字.cpp : Defines the entry point for the console application.
...
public:
	void FindNumsAppearOnce(vector<int> data, int* num1, int *num2) {
		int exclusive_or = 0;
		for (int i = 0; i < data.size(); i++)
			exclusive_or = exclusive_or ^ data[i];
		int index = 1; //从右往前找到两个出现一次的数的不同的第一位
		while ((exclusive_or & index) == 0)
			index = index << 1;
		*num1 = 0;
		*num2 = 0;
		for (int i = 0; i < data.size(); i++)
